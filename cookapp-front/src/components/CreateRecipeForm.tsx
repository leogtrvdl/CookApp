...
export default function CreateRecipeForm({ onClose }: Props) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const res = await fetchWithAuth(`${API_URL}/recipes`, { method: 'POST', body: formData });
  ...
  return (
    <View style={{ flex: 1, backgroundColor: Colors.background }}>
      <View style={{
        ...
        <Text style={{ fontSize: 17, fontWeight: '600', color: Colors.textDark }}>Créer une recette</Text>
      ...
      <ScrollView contentContainerStyle={{ padding: 16, maxWidth: isMobile ? undefined : 640, alignSelf: 'center', width: '100%' }}>
        ...
        >
          <Text style={{ color: Colors.primaryDark, fontSize: 14 }}>📷 Choisir une image</Text>
        </TouchableOpacity>
        {image && (
        ...
        >
          <Text style={{ color: Colors.white, fontWeight: '600', fontSize: 15 }}>Créer la recette</Text>
        </TouchableOpacity>
